/* global React, ReactDOM, PRODUCTS, CATEGORIES, MUSCLES, USE_CASES */
/* global TopBar, FeedHero, ChipRow, ProductCard, FilterSheet, ProductDetail, CatalogScreen, EnquiryScreen, useCatalog, cls, Icon, Silhouette */
...
function App() {
  ...
  const [filters, setFilters] = useState({ muscles: [], useCases: [], capacityMax: 500, sort: 'Featured' });
  ...
  // Show a toast when adding to catalog
  ...
  const openProduct = (id) => setRoute({ name: 'detail', id });
  const goBrowse = () => setRoute({ name: 'browse' });
  const openCatalog = () => setRoute({ name: 'catalog' });
  const openEnquiry = (productId) => setRoute({ name: 'enquiry', id: productId });

  return (
    <div className="app" style={{ ['--snap']: t.scrollSnap ? 'y mandatory' : 'none' }}>
      ...
      <aside className="sidebar-left desktop-only">
        ...
        {CATEGORIES.map(c => (
          ...
          >
            <Icon name={c.id === 'all' ? 'grid' : c.id === 'cardio' ? 'heart' : 'grid'} size={16} />
            {c.label}
      ...
      <main className="main-col-wrap">
        ...
        <div className="main-col desktop-only">
          ...
          <div className="feed-grid">
            {filtered.map(p => (
              <ProductCard key={p.id} p={p} onOpen={() => openProduct(p.id)} catalog={catalog} accent={t.accent} />
            ))}
          </div>
          {filtered.length === 0 && (
      ...
      <aside className="sidebar-right desktop-only">
        <div className="head">
          <h4>Your Catalog</h4>
          <button className="icon-btn"><Icon name="folder" size={16} />
      ...
      {/* Floating CTA (mobile only — desktop has sidebar) */}
      {route.name === 'browse' && (
      ...
      {toast && <div className="toast"><Icon name="check" size={14} /> {toast.text}</div>}
...
function DesktopCatalogPanel({ catalog, onEnquire, onOpen }) {
  ...
  if (items.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: '40px 8px', color: 'var(--text-dim)', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        ...
        <div style={{ fontSize: 13, marginTop: 4, maxWidth: 22, marginLeft: 'auto', marginRight: 'auto' }}>Tap the + on any machine to start.</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
      <div style={{ overflowY: 'auto', flex: 1, marginRight: -10, paddingRight: 10 }}>
        {items.map(({ p, qty }) => (
          <div key={p.id} className="cat-row" style={{ padding: 10, marginBottom: 8 }} onClick={() => onOpen(p.id)}>
    ...
    <TweaksPanel title="Tweaks">
      <TweakSection label="Accent color">
        <TweakColor
          ...
          options={['#E63946', '#FF6B1A', '#C9FF3D', '#3D7BFF', '#F5F1E8']}
        />
      </TweakSection>
      <TweakSection label="Feed behavior">
        <TweakToggle
          label="Snap-scroll cards"
          value={t.scrollSnap}
